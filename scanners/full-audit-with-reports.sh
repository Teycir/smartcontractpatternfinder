...
# Console output
cargo run --release -- scan --days 90 --all-chains --min-severity info --update-templates 7 --output console | tee "$REPORT_DIR/audit_${TIMESTAMP}_console.txt"

# JSON report
cargo run --release -- scan --days 90 --all-chains --min-severity info --update-templates 0 --output json > "$REPORT_DIR/audit_${TIMESTAMP}.json"

# SARIF report (for CI/CD)
cargo run --release -- scan --days 90 --all-chains --min-severity info --update-templates 0 --output sarif > "$REPORT_DIR/audit_${TIMESTAMP}.sarif"
