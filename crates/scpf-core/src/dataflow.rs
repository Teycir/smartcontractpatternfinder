...
impl OldDataFlowAnalyzer {
    ...
    pub fn analyze_function(&mut self, node: Node, source: &str) {
        self.taint_map.clear();
        self.collect_taints(node, source);
        self.propagate_taints(node, source);
        self.check_sinks(node, source);
    }

    fn collect_taints(&mut self, node: Node, source: &str) {
        let mut cursor = node.walk();
        for child in node.children(&mut cursor) {
            if child.kind() == "member_access" {
                let text = child.utf8_text(source.as_bytes()).unwrap_or("");
                if text.starts_with("msg.") {
                    if let Some(prop) = text.strip_prefix("msg.") {
                        let taint_source = match prop {
                            "sender" => Some(TaintSource::MsgSender),
                            "value" => Some(TaintSource::MsgValue),
                            "data" => Some(TaintSource::MsgData),
                            _ => None,
                        };
                        if let Some(src) = taint_source {
                            if let Some(parent) = child.parent() {
                                if parent.kind() == "assignment" {
                                    if let Some(target) = parent.child(0) {
                                        let var = target.utf8_text(source.as_bytes()).unwrap_or("");
                                        self.add_taint(var, src);
                                    }
                                }
                            }
                        }
                ...
                if text == "tx.origin" {
                    if let Some(parent) = child.parent() {
                        if parent.kind() == "assignment" {
                            if let Some(target) = parent.child(0) {
                                let var = target.utf8_text(source.as_bytes()).unwrap_or("");
                                self.add_taint(var, TaintSource::TxOrigin);
                            }
                        }
                    }
                }
            }
            self.collect_taints(child, source);
        }
    }

    fn propagate_taints(&mut self, node: Node, source: &str) {
        ...
        while changed && iterations < 100 {
            ...
            for child in node.children(&mut cursor) {
                if child.kind() == "assignment" {
                    if let (Some(lhs), Some(rhs)) = (child.child(0), child.child(2)) {
                        let target = lhs.utf8_text(source.as_bytes()).unwrap_or("").to_string();
                        let rhs_vars = self.collect_identifiers(rhs, source);
                        let new_sources: HashSet<_> = rhs_vars
                            .iter()
                            .filter_map(|v| self.taint_map.get(v))
                            .flat_map(|t| t.sources.iter().cloned())
                            .collect();
                        if !new_sources.is_empty() {
                            let existing = self.taint_map.get(&target);
                            if existing.map(|e| &e.sources) != Some(&new_sources) {
                                let mut path = rhs_vars
                                    .first()
                                    .and_then(|v| self.taint_map.get(v))
    ...
    fn check_sinks(&mut self, node: Node, source: &str) {
        let mut cursor = node.walk();
        for child in node.children(&mut cursor) {
            if child.kind() == "function_call" {
                if let Some(callee) = child.child(0) {
                    ...
                    if let Some(sink_type) = sink {
                        let args = self.collect_identifiers(child, source);
                        for arg in args {
                            if let Some(taint) = self.taint_map.get(&arg) {
            ...
            self.check_sinks(child, source);
        }
    }

    fn add_taint(&mut self, var: &str, source: TaintSource) {
        let entry = self.taint_map.entry(var.to_string()).or_insert(TaintInfo {
            sources: HashSet::new(),
            propagation_path: vec![var.to_string()],
        });
        entry.sources.insert(source);
    }

    fn collect_identifiers(&self, node: Node, source: &str) -> Vec<String> {
        let mut ids = Vec::new();
        if node.kind() == "identifier" {
            ids.push(node.utf8_text(source.as_bytes()).unwrap_or("").to_string());
        }
        let mut cursor = node.walk();
        for child in node.children(&mut cursor) {
            ids.extend(self.collect_identifiers(child, source));
        }
        ids
    }

    fn calculate_severity(&self, source: &TaintSource, sink: &TaintSink) -> DataFlowSeverity {
        match (source, sink) {
            (TaintSource::TxOrigin, TaintSink::Selfdestruct) => DataFlowSeverity::Critical,
            (TaintSource::MsgSender, TaintSink::DelegateCall) => DataFlowSeverity::Critical,
            (TaintSource::CallData, TaintSink::DelegateCall) => DataFlowSeverity::Critical,
            (TaintSource::FunctionParam(_), TaintSink::DelegateCall) => DataFlowSeverity::High,
            (_, TaintSink::Selfdestruct) => DataFlowSeverity::High,
            (_, TaintSink::DelegateCall) => DataFlowSeverity::High,
            _ => DataFlowSeverity::Medium,
        }
    }
}
