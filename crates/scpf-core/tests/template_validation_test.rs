...
fn test_weak_randomness_detection() {
    ...
    let contract_code =
        std::fs::read_to_string(TEST_CONTRACT_PATH).expect("Failed to read contract");

    let scanner = Scanner::new(templates).expect("Failed to create scanner");
    let results = scanner
        .scan(
            &contract_code,
            PathBuf::from("test_vulnerable_patterns.sol"),
        )
        .expect("Scan failed");
    ...
    for m in &results {
        println!(
            "  - Pattern: {}, Line: {}, Text: {}",
            m.pattern_id, m.line_number, m.matched_text
        );
    }

    let blockhash_findings: Vec<_> = results
        .iter()
        .filter(|m| m.pattern_id.contains("blockhash"))
        .collect();

    assert!(
        !blockhash_findings.is_empty(),
...
fn test_arbitrary_call_detection() {
    ...
    let arbitrary_call_findings: Vec<_> = results
        .iter()
        .filter(|m| m.pattern_id.contains("arbitrary-call") || m.pattern_id.contains("call"))
        .collect();

    assert!(
        !arbitrary_call_findings.is_empty(),
...
fn test_reentrancy_erc20_detection() {
    ...
    let transferfrom_findings: Vec<_> = results
        .iter()
        .filter(|m| {
            println!(
                "    Checking: pattern_id={}, filtered={}",
                m.pattern_id, m.filtered
            );
            m.pattern_id.contains("transferfrom")
                || m.pattern_id.contains("erc20")
                || m.pattern_id.contains("before-state")
        })
        .collect();

    assert!(
        !transferfrom_findings.is_empty(),
...
fn test_signature_replay_detection() {
    ...
    let ecrecover_findings: Vec<_> = results
        .iter()
        .filter(|m| m.pattern_id.contains("ecrecover"))
        .collect();

    assert!(
        !ecrecover_findings.is_empty(),
...
fn test_delegatecall_detection() {
    ...
    let delegatecall_findings: Vec<_> = results
        .iter()
        .filter(|m| m.pattern_id.contains("delegatecall"))
        .collect();

    assert!(
        !delegatecall_findings.is_empty(),
...
fn test_callback_reentrancy_detection() {
    ...
    let callback_findings: Vec<_> = results
        .iter()
